...
if __name__ == "__main__":
    ...
    # print(df_input_xlsx)
    #
    df_P_names_list = df_input_xlsx["Experiment series name posterior (SP8)"].tolist()
    df_class_list = df_input_xlsx["class"].tolist()
    #
    assert len(df_P_names_list) == len(df_class_list), "length of 'A_name_list' and 'class_list' misMatch"
    print(f"len(df_P_names_list) = len(df_class_list) = {len(df_class_list)}\n")



    # *** Start process ***
    for i, fish_name in enumerate(df_P_names_list):
    ...
    log_path_abs = f"{write_log_dir}/[log]{time_stamp}_using_(crop_img_P).json"
