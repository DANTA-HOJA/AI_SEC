...
class BaseNormBFTrainer(BaseObject):
    ...
    def _set_config_attrs(self):
        ...
        assert self.config["dataset"]["file_name"] == "DS_SURF3C_NORMBF.xxx", "`dataset.file_name` in config should be `DS_SURF3C_NORMBF.xxx`"
    ...
    def _set_dataset_df(self):
        ...
        tmp_list = [self.dataset_file_name.split("_")[1],
                    self.dataset_classif_strategy,
                    self.dataset_seed_dir]
        dataset_file = self._processed_di.clustered_files_dict[f"{'_'.join(tmp_list)}"]
        
        if not dataset_file.exists():
            raise FileNotFoundError(f"{Fore.RED}{Back.BLACK} Can't find target dataset file "
                                    f"run `0.5.3.cluster_data.py` to create it. "
                                    f"{Style.RESET_ALL}\n")
    ...
    def _print_trainingset_informations(self):
        ...
        [self._cli_out.write(f"{i} : image_name = {self.train_df.iloc[i]['Brightfield']}") for i in range(5)]
        
        self._cli_out.write(f"valid_data ({len(self.valid_df)})")
        [self._cli_out.write(f"{i} : image_name = {self.valid_df.iloc[i]['Brightfield']}") for i in range(5)]
    ...
    def _one_epoch_training(self, epoch:int):
        ...
        for data in self.train_dataloader:
            ...
            images, images2, labels, dnames = data
