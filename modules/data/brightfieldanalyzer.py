...
class BrightfieldAnalyzer():
    ...
    def __init__(self, zfij_instance:ZFIJ=None) -> None:
        ...
        """ Logger """
        self._logger = init_logger(r"Analyze Brightfield")
    ...
    def run(self):
        """ Actions
        1. Load config: `0.3.analyze_brightfield.toml`
        2. Source: 
            1. Get `lif_scan_root`
            2. Scan `LIF` files
        3. Destination:
        ...
        """ STEP 1. Load config """
        config = load_config("0.3.analyze_brightfield.toml", **self._display_kwargs)
        ...
        """ 3.1. """
        try:
            self.brightfield_processed_dir = self._path_navigator.processed_data.get_processed_dir("brightfield", config, **self._display_kwargs)
            assert_dir_exists(self.brightfield_processed_dir)
            self.mode = "UPDATE"
        except FileNotFoundError:
            create_new_dir(self.brightfield_processed_dir, display_on_CLI=False)
        ...
        for idx in range(series_cnt):
            ...
            if roi_cnt > 0:
                ...
                if roi_cnt == 1:
                    ...
                    """ logger and log_file """
                    self._logger.info(f"      ROI in RoiManager: {roi_cnt}")
                    self._logger.info(f"      #### ERROR : Number of ROI not = 1")
                    # Write Log
    ...
    def find_focused_plane(self, img, save_name:str, save_dir:Path):
        ...
        if slices > 1:
            self._logger.info("      #### WARNING : Number of Slices > 1, run ' Find focused slices ' ") # WARNING:
