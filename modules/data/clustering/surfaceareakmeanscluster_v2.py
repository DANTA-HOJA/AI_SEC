...
class SurfaceAreaKMeansCluster():
    ...
    def set_attrs(self, config_file:Union[str, Path]):
        ...
        if self.processed_data_instance.data_xlsx_path:
            self.orig_xlsx_path = self.processed_data_instance.data_xlsx_path
        else: 
            raise FileNotFoundError(f"{Fore.RED}{Back.BLACK} Can't find `data.xlsx` please use `self.create_data_xlsx()` to create it. {Style.RESET_ALL}\n")
