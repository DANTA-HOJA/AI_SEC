...
class MpCamGalleryCreator():
    
    def __init__(self, config_path:Path, max_str_len_queues, lock,
                       init_sig_queue, pbar_sig_queue) -> None:
        ...
        self.lock = lock
        self.init_sig_queue = init_sig_queue
        self.pbar_sig_queue = pbar_sig_queue
        self.max_str_len_queues = max_str_len_queues
        
        self.process_name = multiprocessing.current_process().name
        self.process_id = multiprocessing.current_process().pid
        
        with self.lock:
            max_str_len_dict = self.max_str_len_queues.get()
            if len(self.process_name) > max_str_len_dict["process_name"]:
                max_str_len_dict["process_name"] = len(self.process_name)
            self.max_str_len_queues.put(max_str_len_dict)
        ...
        with self.lock:
            init_sig_queue.put(f"{Fore.MAGENTA}{self.process_name} "
                            f"{Fore.BLUE}{self.process_id} "
                            f"{Fore.GREEN}initial{Style.RESET_ALL} completed")
            init_sig_queue.put(f"{self}\n")
        
        # End of `__init__` -------------------------------------------------------------------------------------
    
    
    def gen_batch_cam_gallery(self, fish_dsname_list:List[str]):
    ...
    def gen_single_cam_gallery(self, fish_dsname:str):
        ...
        with self.lock:
            max_str_len_dict = self.max_str_len_queues.get()
            self.pbar_sig_queue.put(f"Generate {Fore.YELLOW}'{fish_dsname:{max_str_len_dict['fish_dsname']}}'{Style.RESET_ALL} "
                                    f"( {Fore.MAGENTA}{self.process_name:{max_str_len_dict['process_name']}}{Style.RESET_ALL} ) ")
            self.max_str_len_queues.put(max_str_len_dict)
        ...
        # pbar_signal
        with self.lock: self.pbar_sig_queue.put(1)
