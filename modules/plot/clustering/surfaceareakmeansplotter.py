...
class SurfaceAreaKMeansPlotter():
    ...
    def __init__(self, display_on_CLI=True) -> None:
        ...
        self.fish_batch_mark:List[int] # 建立一個 list 標記 data 對應的 batch
        self.fish_day_mark:List[int]   # 建立一個 list 標記 data 對應的 day (dpf)
        self.fish_day_cnt:Union[Counter, OrderedDict]
    ...
    def set_attrs(self, config_file:Union[str, Path]):
        ...
        self._set_old_xlsx_attrs()
        self._set_kmean_attrs()
        ...
        self._set_fish_batch_mark()
        self._set_fish_day_mark()
    
    
    
    def _set_config_attrs(self, config_file:Union[str, Path]):
        """ Set below attributes
            1. `self.old_classdiv_xlsx_path`
            2. `self.random_seed`
            3. `self.n_class`
            4. `self.labels`
            5. `self.cluster_with_log_scale`
            6. `self.log_base`
            7. `self.x_axis_log_scale`
        """
        config = load_config(config_file, cli_out=self._cli_out)
        
        """ [old_classdiv_xlsx] """
        self.old_classdiv_xlsx_path = config["old_classdiv_xlsx"]["full_path"]
        
        """ [param] """
        self.random_seed = config["param"]["random_seed"]
        
        """ [param.cluster] """
        self.n_class = config["param"]["cluster"]["n_class"]
        self.labels = config["param"]["cluster"]["labels"] # small -> big, e.g. ["S", "M", "L"]
        self.cluster_with_log_scale = config["param"]["cluster"]["cluster_with_log_scale"]
        
        """ [param.log_scale] """
        self.log_base = config["param"]["log_scale"]["base"]
        self.x_axis_log_scale = config["param"]["log_scale"]["x_axis_log_scale"]
    
    
    
    def _set_orig_xlsx_attrs(self):
        """ Set below attributes
            1. `self.orig_xlsx_path`
            2. `self.orig_xlsx_df`
            3. `self.orig_xlsx_path_split`
        """
        def set_attr1():
        ...
        self.orig_xlsx_df = pd.read_excel(self.orig_xlsx_path, engine = 'openpyxl')
    ...
    def _set_clustered_xlsx_attrs(self):
        """ Set below attributes
            ...
            2. `self.clustered_xlsx_name`
            3. `self.clustered_xlsx_path`
            4. `self.clustered_xlsx_df`
        """
        def set_attr1():
        ...
        self.clustered_xlsx_dir = set_attr1()
        self.clustered_xlsx_name = set_attr2()
        self.clustered_xlsx_path = self.clustered_xlsx_dir.joinpath(f"{{{self.clustered_xlsx_name}}}_data.xlsx")
        self.clustered_xlsx_df = None # assigned after `gen_clustered_xlsx_df()`
    
    
    
    def _set_old_xlsx_attrs(self):
        """
        """
        def set_attr1():
            if self.old_classdiv_xlsx_path:
                path = Path(self.old_classdiv_xlsx_path[0])
            else:
                path = None
            return path
        
        def check_old_classdiv_n_class(name_split:List[str], list_idx:int):
            """ 'SURF10C' -> int(10) """
            match = re.search(r'\d+', name_split[list_idx]) # 匹配一個或多個連續數字
            if match: 
                number = int(match.group()) # 如果找到匹配，取出匹配的結果
            else:
                raise ValueError(f"File name: '{self.old_classdiv_xlsx_name}' is not correct format.")
            
            assert number == self.n_class, (f"`n_class` in `old_classdiv_xlsx` are not match, expect {self.n_class}, but got {number}, "
                                            f"file: '{self.old_classdiv_xlsx_path.resolve()}'")
            return number
        
        def set_attr2(name_split:List[str], list_idx:int):
            """ '050STDEV' -> '0.5_STDEV' """
            stdev = int(name_split[list_idx].replace("STDEV", ""))/100
            return f"{stdev}_STDEV"
        
        def set_attr3():
            comp_dir = self.clustered_xlsx_dir.joinpath("compare_clustering", "KMeans_comp_STDEV")
            if self.x_axis_log_scale:
                path = comp_dir.joinpath(f"x-axis in LOG{self.log_base} scale")
            else:
                path = comp_dir.joinpath("x-axis in ORIG scale")
            create_new_dir(path)
            return path
        
        self.old_classdiv_xlsx_path = set_attr1()
        
        if self.old_classdiv_xlsx_path:
            """ / """
            self.old_classdiv_xlsx_name = str(self.old_classdiv_xlsx_path).split(os.sep)[-1] # '{3CLS_SURF_050STDEV}_data.xlsx'
            
            old_classdiv_xlsx_name_split = re.split("{|_|}", self.old_classdiv_xlsx_name) # ['', '3CLS', 'SURF', '050STDEV', '', 'data.xlsx']
            check_old_classdiv_n_class(old_classdiv_xlsx_name_split, 1)
            self.old_classdiv_strategy = set_attr2(old_classdiv_xlsx_name_split, 3)
            self.compared_cluster_img_dir = set_attr3()
            
            self.old_classdiv_xlsx_df = pd.read_excel(self.old_classdiv_xlsx_path, engine = 'openpyxl')
    ...
    def __repr__(self):
        part1 = (f'self.dataset_id              : {self.dataset_id}\n'
                 ...
                 f'self.n_class                 : {self.n_class}\n'
                 f'self.labels                  : {self.labels}\n'
                 f'self.random_seed             : {self.random_seed}\n'
        ...
        output = (part1 + part2) if self.old_classdiv_xlsx_path else part1
    ...
    def gen_clusters_idx2str(self): # dependency: self.clusters_max_area
        self.clusters_idx2str = { cls_idx: cls_str for (cls_idx, _), cls_str in zip(self.clusters_max_area.items(), self.labels) }
