...
umap = UMAP(n_components=2, random_state=rand_seed).fit_transform(X_scaled)
...
ax.set_title(f"UMAP ({img_mode} image, first {n_pca} PCA components)")
...
umap_pt_path = dst_dir.joinpath(f"{notebook_name}.{img_mode}.json")
with open(umap_pt_path, 'w') as f_writer:
    json.dump(data, f_writer, indent=4)

print(f"Save UMAP results to : '{umap_pt_path.parent}'")